...
<template>
  <div class="container mx-auto p-4">
    ...
    <router-link
      to="/tenencias/create"
      class="bg-blue-500 text-white px-4 py-2 mb-4 rounded inline-block"
      >Cargar Operacion</router-link
    >
    <div v-if="datosCargados">
      <table class="min-w-full border-collapse border border-gray-200">
        <thead>
          <tr>
            <th class="border border-gray-200 px-4 py-2 bg-gray-100 text-left">
              Nombre
            </th>
            <th class="border border-gray-200 px-4 py-2 bg-gray-100 text-left">
              Ticker
        ...
        <tbody>
          ...
          >
            ...
            <td class="border border-gray-200 px-4 py-2">
              {{ formatDate(tenencia.fecha_compra) }}
            </td>
            <td class="border border-gray-200 px-4 py-2">
              {{ roundToTwoDecimals(tenencia.precio_compra) }}
            </td>
            <td class="border border-gray-200 px-4 py-2">
              {{ roundToTwoDecimals(tenencia.precio_actual) }}
            </td>
            <td class="border border-gray-200 px-4 py-2">
              {{ roundToTwoDecimals(tenencia.precio_actual_usd) }}
            </td>
            <td class="border border-gray-200 px-4 py-2">
              {{ roundToTwoDecimals(tenencia.ganancia) }}
            </td>
            <td class="border border-gray-200 px-4 py-2">
              {{ roundToTwoDecimals(tenencia.ganancia_porcentaje) }}
            </td>
            <td class="border border-gray-200 px-4 py-2">
              {{ roundToTwoDecimals(tenencia.ganancia_usd) }}
            </td>
            <td class="border border-gray-200 px-4 py-2">
              {{ roundToTwoDecimals(tenencia.ganancia_porcentaje_usd) }}
            </td>
          </tr>
        </tbody>
        <tfoot>
          <tr class="bg-gray-100">
            ...
            <td class="border border-gray-200 px-4 py-2">
              {{ total_ganancia_porcentaje }}
            </td>
            <td class="border border-gray-200 px-4 py-2">
              {{ total_ganancia_usd }}
            </td>
            <td class="border border-gray-200 px-4 py-2">
              {{ total_ganancia_porcentaje_usd }}
            </td>
          </tr>
        </tfoot>
      </table>
      <DataView :value="tenencias">
